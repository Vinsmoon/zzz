...
def date_if(prefer_sit,driver):
    ...
    if times_roll and len(times_roll) > 0:
        ...
        if all(time in full_day_times for time in get_time_range(full_day_times,"15:00","19:00")):
            day_type = 3
...
def choose_it(driver, sit_avilable, idx, reading_room, day_type, max_attempts=500):
    ...
    bool: 预约是否成功
    """
    dir_time = {3: [['15:00', '19:00'], ['15:00', '19:00']], 2: [['14:00', '18:00'], ['18:00', '22:00']]}
        ...
        try:
            ...
            if reserve_box:
                ...
                if times_roll:
                    ...
                    if all(time in time_texts for time in get_time_range(full_day_times,"15:00","19:00")):
                        print(f"找到全天可预约座位: {i}")
                        found_full_day = True
                        day_type = 3
        ...
        try:
            ...
            if times_roll and len(times_roll) > 0:
                ...
                if all(time in time_texts for time in
                       get_time_range(full_day_times, "15:00", "19:00")):
...
def main():
    """主函数：循环登录多个账号并执行操作"""
    account_password4 = {
        "4032430171": "969a7s2d7f",
        "4032430171": "969a7s2d7f"
    }
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "三楼理科书库", "34"]}
    user = "自定义"
